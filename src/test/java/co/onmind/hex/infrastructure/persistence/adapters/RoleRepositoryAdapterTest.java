...
class RoleRepositoryAdapterTest {
    ...
    @Test
    @DisplayName("Should find roles by name pattern successfully")
    void shouldFindRolesByNamePattern() {
        // Given
        String pattern = "ADM";
        List<RoleEntity> entities = List.of(testEntity);
        List<Role> roles = List.of(testRole);

        when(jpaRepository.findByNameContainingIgnoreCase(pattern)).thenReturn(entities);
        when(entityMapper.toDomainList(entities)).thenReturn(roles);

        // When
        List<Role> result = repositoryAdapter.findByNameContainingIgnoreCase(pattern);

        // Then
        assertThat(result).hasSize(1);
        verify(jpaRepository).findByNameContainingIgnoreCase(pattern);
    }
}